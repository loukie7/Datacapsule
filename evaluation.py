"""
Mini-React 评估处理器模块
将原dspy_evaluation.py中的功能迁移到mini-react
"""
import os
from dotenv import load_dotenv
from loguru import logger

from miniReact import Signature, InputField, OutputField, ChainOfThought, Module
from core.llm_manager import get_training_manager

# 确保环境变量已加载
load_dotenv(override=True)

class BiologicalRetrievalEvaluationSignature(Signature):
    """评估生物检索任务的推理步骤质量的签名"""
    def __init__(self):
        super().__init__(
            input_fields={
                "question": InputField(desc="用户的查询问题"),
                "standard_reasoning": InputField(desc="标准的推理步骤"),
                "predicted_reasoning": InputField(desc="模型产生的推理步骤")
            },
            output_fields={
                "evaluation_score": OutputField(desc="评分(0-100)"),
                "evaluation_feedback": OutputField(desc="详细的评分解释，包括各个方面的得分")
            },
            instructions="""
...
class LLMBiologicalEvaluator(Module):
    """大模型生物检索评估器"""
    def __init__(self, eval_lm):
        ...
        # 使用思维链方式增强评估能力
        self.eval_chain = ChainOfThought(BiologicalRetrievalEvaluationSignature())
        if eval_lm:
            self.eval_chain.lm = eval_lm
    ...
    def forward(self, example, prediction):
        ...
        if not hasattr(example, 'reasoning') or not hasattr(prediction, 'reasoning'):
            # 简单匹配评估
            return 1.0 if hasattr(example, 'answer') and hasattr(prediction, 'answer') and example.answer == prediction.answer else 0.0
...
class EvaluationProcessor:
    ...
    def __init__(self):
        ...
        try:
            ...
            logger.info("评估处理器已初始化")
        except Exception as e:
            logger.error(f"初始化评估处理器失败: {str(e)}")
    ...
    def llm_biological_metric(self, example, pred, trace=None, frac=1.0):
        ...
        try:
            # 创建评估器实例，传入评估模型
            evaluator = LLMBiologicalEvaluator(self.training_manager)

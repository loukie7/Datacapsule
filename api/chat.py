...
from services import SSEService
...
def init_services(inference_svc: InferenceService, sse_svc: SSEService):
    """初始化服务实例"""
    global inference_service, sse_service
    inference_service = inference_svc
    sse_service = sse_svc
...
async def chat(
    ...
    try:
        ...
        # 更新预测器版本
        predictor_version = inference_service.get_version()
        ...
        if version and version != predictor_version:
            ...
            inference_service.load_model(file_path)
            
            # 更新 predictor_version
            predictor_version = version
            inference_service.set_version(version)
        ...
        if stream == 1:
            ...
            inference_res = await inference_service.predict(prompt)
            content = inference_res.answer
            reasoning = inference_res.reasoning
            return ResponseWrapper(
                status_code=200, 
                detail="success", 
